...
# ------
# Parameters to ease resource usage:
# ------
MAX_PAGES = 5  # Only process the first 5 pages of a PDF

# Use fewer threads to reduce load.
THREAD_POOL_WORKERS = 2

# -------------------------
# Pipeline Functions with minor modifications
# -------------------------

def pdf_to_images(pdf_path, output_dir, fixed_length=1080, max_pages=MAX_PAGES):
    log_message(f"Converting PDF to images at fixed length {fixed_length}px (max {max_pages} pages)...")
...
# Cache the creation of the YOLO model to avoid re-loading
@st.cache_resource(show_spinner=True)
def get_yolo_model(weight="best_small_yolo11_block_etraction.pt"):
    return YOLO(weight)

class BlockDetectionModel:
    def __init__(self, weight="best_small_yolo11_block_etraction.pt", device=None):
        self.device = "cuda" if (device is None and torch.cuda.is_available()) else "cpu"
        self.model = get_yolo_model(weight).to(self.device)
    ...
    def predict_batch(self, images_dir):
        ...
        # Process images sequentially to reduce peak CPU usage.
...
def process_with_gemini(image_paths, prompt):
    log_message(f"Processing {len(image_paths)} images with Gemini OCR (in bulk)...")
    contents = [prompt]
    for path in image_paths:
        try:
            with Image.open(path) as img:
                # Resize for faster processing (use a lower resolution if needed)
...
if uploaded_pdf and not st.session_state.processed:
    if st.sidebar.button("Run Processing Pipeline"):
        ...
        # Process PDF using limited concurrency and only the first MAX_PAGES pages.
        with concurrent.futures.ThreadPoolExecutor(max_workers=THREAD_POOL_WORKERS) as executor:
            future_low = executor.submit(pdf_to_images, pdf_path, LOW_RES_DIR, 662, MAX_PAGES)
            future_high = executor.submit(pdf_to_images, pdf_path, HIGH_RES_DIR, 4000, MAX_PAGES)
            low_res_paths = future_low.result()
            high_res_paths = future_high.result()
        log_message("PDF conversion completed.")

        log_message("Running YOLO detection on low-res images...")
        yolo_model = BlockDetectionModel()
...
6. Drawing_Number
7. Project_Number
8. Revision_Number (numeric or 'N/A')
9. Scale
10. Architects (list, use ['Unknown'] if not found)
11. Notes_on_Drawing

Key Requirements:
- If any field is missing, return an empty string ('') or 'N/A'
- Return a valid JSON object containing these fields (and no additional commentary).
Below is an example JSON:
{
    "Purpose_Type_of_Drawing": "Architectural",
    "Client_Name": "Example Client",
    "Project_Title": "Example Project",
    "Drawing_Title": "Sample Drawing",
    "Floor": "1",
    ...
    "Architects": ["Unknown"],
    "Notes_on_Drawing": "Example notes."
}
"""
        ...
        log_message("Setting up retrievers and RAG pipeline...")
