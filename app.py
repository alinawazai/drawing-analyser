...
DATA_DIR = "data"
HIGH_RES_DIR = os.path.join(DATA_DIR, "500_dpi")  # Ensure higher resolution
OUTPUT_DIR = os.path.join(DATA_DIR, "output")

# Set up basic logging (optional)
logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s')

# Minimal log function: simply print to sidebar.
def log_message(msg):
...
def pdf_to_images(pdf_path, output_dir, fixed_length=4000):
    log_message(f"Converting PDF to high-res images at fixed length {fixed_length}px...")
    if not os.path.exists(pdf_path):
        raise FileNotFoundError(f"PDF not found: {pdf_path}")
    base_name = os.path.splitext(os.path.basename(pdf_path))[0]
    if not os.path.exists(output_dir):
...
def process_with_gemini(image_paths, prompt):
    log_message(f"Processing {len(image_paths)} images with Gemini OCR...")
    contents = [prompt]
    for path in image_paths:
        try:
            with Image.open(path) as img:
    ...
    time.sleep(4)  # Simple rate-limiting
    response = client.models.generate_content(model="gemini-2.0-flash", contents=contents)
    log_message("Gemini OCR response received.")
    resp_text = response.text.strip()
    try:
        return json.loads(resp_text)
    except json.JSONDecodeError:
...
if query and st.session_state.processed:
    ...
    try:
        ...
        for doc in results:
            ...
            img_path = doc.metadata.get("drawing_path", "")
            if img_path and os.path.exists(img_path):
                st.image(Image.open(img_path), width=1000)  # Increased image size for better visibility
    except Exception as e:
        st.error(f"Search failed: {e}")
