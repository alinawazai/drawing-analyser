...
def log_message(msg):
    """Convenience function to store log messages in session_state logs."""
    st.session_state.logs.append(msg)
    # Re-render the logs
    col1.markdown("\n".join(st.session_state.logs))
...
def pdf_to_images(pdf_path, output_dir, fixed_length=1080):
    """Convert each page of the PDF into an image, scaled to `fixed_length` width."""
    log_message(f"Converting PDF to images at fixed length {fixed_length}px...")
    if not os.path.exists(pdf_path):
        raise FileNotFoundError(f"PDF not found: {pdf_path}")
    ...
    file_paths = []
    # Replacing concurrency with a simple for-loop
...
with col1:
    ...
    if uploaded_pdf and not st.session_state.processed:
        if st.button("Run Processing Pipeline"):
            ...
            log_message("Setting up RAG pipeline with re-ranking...")
            compressor = CohereRerank(model="rerank-multilingual-v3.0", top_n=5)
            compression_retriever = ContextualCompressionRetriever(
